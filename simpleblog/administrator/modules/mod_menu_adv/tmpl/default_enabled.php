...
if ($im || $mm || $pm || $tm || $lm)
{
	...
	if ($mm)
	{
		...
		$modules=ModMenuAdvHelper::getModules();

		foreach ($modules as $module) {
			$lang->load($module->element . '.sys', JPATH_SITE, null, false, true);
			$menu->addChild(new JMenuNode(JText::_($module->element), 'index.php?option=com_modules&task=module.add&eid='.$module->extension_id));
		}
		$menu->getParent();
		...
		$modules=ModMenuAdvHelper::getModules(1);
		$lang = JFactory::getLanguage();

		foreach ($modules as $module) {
			$lang->load($module->element . '.sys', JPATH_ADMINISTRATOR, null, false, true);
			$menu->addChild(new JMenuNode(JText::_($module->name), 'index.php?option=com_modules&task=module.add&eid='.$module->extension_id));
		}
		$menu->getParent();
